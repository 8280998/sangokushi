                ...
                if cursor == 'pointer':
                    ...
                    if button.is_visible() and button.is_enabled():
                        ...
                        if success:
                            ...
                            # 将武将数量写入到 test.txt 文件中
                            with open("test.txt", "a") as file:
                                   file.write(f"Bonus Rate Table 元素数量: {brt_count}\n{username}\n")
...
def daily_bonus_task(page):
    logging.info("尝试通过 INTERCEPTION 按钮进入页面")
    if page.is_visible("img[src*='d84df04b.png']"):
        page.click("img[src*='d84df04b.png']")
        logging.info("关闭吕布弹窗")
        time.sleep(2)

    # 查找并点击 INTERCEPTION 按钮
    interception_button = page.locator("button:has-text('Interception')")
    interception_button.wait_for(state="visible", timeout=5000)  # 等待第一个按钮可见
    if interception_button.is_visible() and interception_button.is_enabled():
        cursor_style = interception_button.evaluate("el => window.getComputedStyle(el).cursor")
        if cursor_style == "pointer":
            interception_button.scroll_into_view_if_needed()
            interception_button.click(force=True)  # 强制点击
            logging.info("点击 INTERCEPTION 按钮")
            page.wait_for_load_state("load")  # 等待页面加载
        else:
            logging.warning("INTERCEPTION 按钮不可点击，光标样式不是 pointer")
    else:
        logging.warning("未找到可用的 INTERCEPTION 按钮，可能需要检查选择器或页面状态")
        time.sleep(2)  # 等待页面稳定
    ...
    # 处理第一次 Intercept 按钮
    first_intercept_button = page.locator("button:has-text('Intercept')")
    first_intercept_button.wait_for(state="visible", timeout=5000)  # 等待第一个按钮可见
    if first_intercept_button.is_visible() and first_intercept_button.is_enabled():
        cursor_style = first_intercept_button.evaluate("el => window.getComputedStyle(el).cursor")
        if cursor_style == "pointer":
            first_intercept_button.scroll_into_view_if_needed()
            first_intercept_button.click(force=True)  # 强制点击
            logging.info("点击第一个 Intercept 按钮")
            time.sleep(2)  # 等待弹窗加载
        else:
            logging.warning("第一个 Intercept 按钮不可点击，光标样式不是 pointer")

    # 处理第二次 Intercept 按钮
    second_intercept_button = page.locator("button:has-text('Intercept')").nth(1)  # 定位第二个按钮
    second_intercept_button.wait_for(state="visible", timeout=5000)  # 等待第二个按钮可见
    if second_intercept_button.is_visible() and second_intercept_button.is_enabled():
        cursor_style = second_intercept_button.evaluate("el => window.getComputedStyle(el).cursor")
        if cursor_style == "pointer":
            second_intercept_button.scroll_into_view_if_needed()
            second_intercept_button.click(force=True)  # 强制点击
            logging.info("点击第二个 Intercept 按钮")
            time.sleep(3)  # 点击后等待3秒
        else:
            logging.warning("第二个 Intercept 按钮不可点击，光标样式不是 pointer")
...
def process_user(username, password):
    with sync_playwright() as p:
        ...
        try:
            ...
            take_screenshot(page, prefix=f"error_{username}")  # 保留截图
        finally:
            browser.close()
